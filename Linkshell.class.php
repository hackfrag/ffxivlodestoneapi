...
class Linkshell {
   ...
   private function buildMembers(){
      ...
      foreach($html->find('div.base_footer div.base_body div.base_inner div.area_inner_header div.area_inner_footer div.area_inner_body div.table_black_border_bottom table tbody tr') as $member_row) {
         ...
         if($member_row->find('td div.player_name_area span.ic_master',0)){
            $rank = "Master";
         } elseif($member_row->find('td div.player_name_area span.ic_leader',0)){
            $rank = "Leader";
         } else {
            $rank = "Member";            
         }
         ...
         if($member_row->find('td div.col3box div.col3box_center div',1)){
         ...
         if($member_row->find('td div.col3box div.col3box_right div',1)){
         ...
         $member["class"]["lvl"]  = $member_row->find('td div.col3box div.col3box_left div',1)->plaintext;
   ...
   private function getNext() {
      if($next = $this->linkshellData->find('div.base_footer div.base_body div.base_inner div.mb10 div.pager div.pagination ul li.next a',0)) {
         $ffxivConnect = new FFXIVConnect($this->userAgent, $this->utf8);
         $this->linkshellData = $ffxivConnect->getLinkshell($this->lodestoneID, $this->region, $next->href);
         $this->buildMembers();
      }
      return FALSE;
   }
